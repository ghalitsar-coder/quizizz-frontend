...
export default function HostRoomPage() {
  ...
  const [gameStatus, setGameStatus] = useState<"LOBBY" | "PLAYING" | "ENDED">(
    "LOBBY"
  );
  const [players, setPlayers] = useState<string[]>([]);
  const [currentQuestion, setCurrentQuestion] =
    useState<QuestionStartPayload | null>(null);
  ...
  useEffect(() => {
    ...
    socket.on(
      "player_joined",
      (data: { name: string; totalPlayers: number; players?: string[] }) => {
            ...
            <CardContent>
              {players.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Minta siswa memasukkan kode: <strong>{roomCode}</strong>
                  </p>
                </div>
              ) : (
                ...
                <div key={key} className="space-y-1">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-6 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(answerStats[key] / maxStats) * 100}%`,
                      }}
                    />
                  </div>
                </div>
